    ...
    (function () {
        ...
        function inc () {
            ...
            if (!isNaN(to) && amount < to) {
                ...
                this._field.val(amount.toFixed(this._defaults.precision));
            } else if (isNaN(to)) {
                amount += step;
                this._field.val(amount.toFixed(this._defaults.precision));
            }
        ...
        function init () {
            ...
            this._field.on('change blur', function () {
                var val = $(this).val(),
                    from = parseFloat(self._field.data('from')),
                    to = parseFloat(self._field.data('to'));

                if (parseFloat(val) > to && !isNaN(to)) {
                    self._field.val(self._field.data('to'));
                } else if (parseFloat(val) < from && !isNaN(from)) {
                    self._field.val(self._field.data('from'));
                } else if (val === '' && !isNaN(from)) {
                    self._field.val(from);
                } else {
                    self._field.val(self._field.data('initial') || 0);
                }
            });
            ...
            this._field.on('input', function (e) {
                ...
                if (!simpla.helpers.isNumeric($(this).val()) && $(this).val() !== '' && ($(this).val() !== '-' && (from < 0 || to < 0))) {
